...
def train(parameters, config, gpu_list, do_test=False, local_rank=-1, **params):
    ...
    if "t5xxl_to_t5base" in (config.get("output","model_name")).lower() or "t5xxl_to_robertabase" in (config.get("output","model_name")).lower():
        #model_AE = AE_1_layer_mutiple_100(dim_0=409600,dim_1=1024,dim_2=76800).to(device)
    ...
    if record_train_epoch!=0:
        trained_epoch = record_train_epoch
    ...
    for epoch_num in range(trained_epoch, epoch):
        ...
        if current_epoch % test_time == 0:
            with torch.no_grad():
                ...
                if "T5" in config.get("model","model_base"):
                    acc_result_eval = valid(model, parameters["valid_dataset"], current_epoch, writer, config, gpu_list, output_function, AE=model_AE)
                else:
