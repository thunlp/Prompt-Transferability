...
def train(parameters, config, gpu_list, do_test=False, local_rank=-1):
    ...
    for dataset in config.get("data","train_dataset_type").strip().split(","):
        for data in parameters["train_dataset_"+str(dataset)]:
            try:
                dataset_all["train_dataset_"+str(dataset)].append([data])
            except:
                dataset_all["train_dataset_"+str(dataset)]=[data]

        #dataset_all["train_dataset_"+str(dataset)] = {data for data in parameters["train_dataset_"+str(dataset)]}
        #dataset_all = {"train_dataset_"+str(dataset):data for data in parameters["train_dataset_"+str(dataset)]}
    ...
    total_len_max = max([len(v) for key,v in dataset_all.items()])
    ...
    max_length=0
    #max_length=331
    for idx, dataset in enumerate(dataset_all):

        #print("====")
        #if model == "train":
        #for line in parameters[dataset]:
        for line in dataset:
    ...
    for epoch_num in range(trained_epoch, epoch):
        ...
        #############
        #Range dataset and make it same length
        #############
        #Task
        taskdict_id_object=dict()
        for name, dataset in dataset_all.items():
            ################
            ##Build shuffle
            choose_id=[1]*total_len+[0]*(len(dataset)-total_len)
            random.shuffle(choose_id)
            ################
            #dataset: torch.utils.data.dataloader.DataLoader object
            for idx, data in enumerate(dataset):
                ...
                for key in data.keys():
                    if key not in ['inputx','mask','label']:
                        data.pop(key, None)
                        continue
                    if isinstance(data[key], torch.Tensor):

                        if key == "inputx":
                            pad_gap = max_length-int(data[key].shape[-1])
                            if pad_gap!=0:
                                pad_id_input = torch.ones( int(data[key].shape[0]), pad_gap, dtype=int)
                                data[key] = torch.cat([data[key],pad_id_input],dim=1)
                            else:
                                pass
                            ...
                            pad_gap = max_length-int(data[key].shape[-1])
                            if pad_gap!=0:
                                pad_id_mask = torch.zeros(int(data[key].shape[0]),max_length-int(data[key].shape[-1]))
                                data[key] = torch.cat([data[key],pad_id_mask],dim=1)
                            else:
                                pass
                        elif key == "label":
                            pass
                        ...
                        ######
                try:
                    taskdict_id_object[idx].append([data])
                except:
                    taskdict_id_object[idx] = [data]
        #############
        #############
        ...
        for idx, tasklist in taskdict_id_object.items():
            #tasklist --> cuda
