...
class ethicsdeontologyDataset(Dataset):
    def __init__(self, config, mode, encoding="utf8", *args, **params):
        ...
        if mode == "test":
            #self.data = [{"sent": ins[1].strip() + ins[2].strip()} for ins in data]
            self.data = [{"sent1": ins[1].strip(), "sent2": ins[2].strip()} for ins in data]
        else:
            #self.data = [{"sent": ins[1].strip(), "label": int(ins[0].strip())} for ins in data]
            self.data = [{"sent1": ins[1].strip(), "sent2": ins[2].strip(), "label": int(ins[0])} for ins in data]
