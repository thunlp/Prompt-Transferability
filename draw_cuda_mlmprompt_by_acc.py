...
counter = 0
#dataset
for MODEL_1 in ["agnewsPromptRoberta_mlm_","cs_wikiPromptRoberta_mlm_","IMDBPromptRoberta_mlm_", "sciercPromptRoberta_mlm_", "SST2PromptRoberta_mlm_"]:
    for S_1 in ["s1","s2"]:
        ...
        domain_ten = list()
        #prompt
        for MODEL_2 in ["agnewsPromptRoberta_mlm_","cs_wikiPromptRoberta_mlm_","IMDBPromptRoberta_mlm_", "sciercPromptRoberta_mlm_", "SST2PromptRoberta_mlm_"]:
            for S_2 in ["s1","s2"]:
                file_name = "result/"+MODEL_1+S_1+"/result_"+MODEL_2+S_2+".json"
                with open(file_name) as file:
                    file = json.load(file)
                    acc = float(json.loads(file)["acc"])*100
