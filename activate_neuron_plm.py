...
if __name__ == "__main__":
    ...
    outputs_query=[[] for _ in range(12)]
    def save_ppt_outputs_hook_query(n):
        def fn(_,__,output):
            outputs_query[n].append(output.detach().to("cpu"))
        return fn

    outputs_key=[[] for _ in range(12)]
    def save_ppt_outputs_hook_key(n):
        def fn(_,__,output):
            outputs_key[n].append(output.detach().to("cpu"))
        return fn

    outputs_value=[[] for _ in range(12)]
    def save_ppt_outputs_hook_value(n):
        def fn(_,__,output):
            outputs_value[n].append(output.detach().to("cpu"))
        return fn

    outputs_attdense=[[] for _ in range(12)]
    def save_ppt_outputs_hook_attdense(n):
        def fn(_,__,output):
            outputs_attdense[n].append(output.detach().to("cpu"))
        return fn

    outputs_attlayer=[[] for _ in range(12)]
    def save_ppt_outputs_hook_attlayer(n):
        def fn(_,__,output):
            outputs_attlayer[n].append(output.detach().to("cpu"))
        return fn

    outputs_intermediate=[[] for _ in range(12)]
    def save_ppt_outputs_hook_intermediate(n):
        def fn(_,__,output):
            outputs_intermediate[n].append(output.detach().to("cpu"))
        return fn

    outputs_dense=[[] for _ in range(12)]
    def save_ppt_outputs_hook_dense(n):
        def fn(_,__,output):
            outputs_dense[n].append(output.detach().to("cpu"))
        return fn

    outputs_layernorm=[[] for _ in range(12)]
    def save_ppt_outputs_hook_layernorm(n):
        def fn(_,__,output):
            outputs_layernorm[n].append(output.detach().to("cpu"))
        return fn


    for n in range(12):
        #这里面提取feature的模组可以改变，这里因为我自定义模型的原因要两层roberta
        model.encoder.roberta.encoder.layer[n].attention.self.query.register_forward_hook(save_ppt_outputs_hook_query(n))
        model.encoder.roberta.encoder.layer[n].attention.self.key.register_forward_hook(save_ppt_outputs_hook_key(n))
        model.encoder.roberta.encoder.layer[n].attention.self.value.register_forward_hook(save_ppt_outputs_hook_value(n))
        model.encoder.roberta.encoder.layer[n].attention.output.dense.register_forward_hook(save_ppt_outputs_hook_attdense(n))
        model.encoder.roberta.encoder.layer[n].attention.output.LayerNorm.register_forward_hook(save_ppt_outputs_hook_attlayer(n))
        model.encoder.roberta.encoder.layer[n].intermediate.register_forward_hook(save_ppt_outputs_hook_intermediate(n))
        model.encoder.roberta.encoder.layer[n].output.dense.register_forward_hook(save_ppt_outputs_hook_dense(n))
        model.encoder.roberta.encoder.layer[n].output.LayerNorm.register_forward_hook(save_ppt_outputs_hook_dense(n))
    ...
    #merge 17 epoch
    for k in range(12):
        print(outputs_key[k][0][0].shape)
        print(outputs_query[k][0][0].shape)
        print(outputs_value[k][0][0].shape)
        print(outputs_attdense[k][0][0].shape)
        #print(outputs_attlayer[k][0][0].shape)
        #print(outputs_layernorm[k][0][0].shape)
        print(outputs_intermediate[k][0][0].shape)
        print(outputs_dense[k][0][0].shape)

        #output = torch.cat((outputs_key[k][0],outputs_query[k][0],outputs_value[k][0],outputs_attdense[k][0],outputs_attlayer[k][0],outputs_layernorm[k][0],outputs_intermediate[k][0],outputs_dense[k][0]),2)
        output = torch.cat((outputs_key[k][0],outputs_query[k][0],outputs_value[k][0],outputs_attdense[k][0],outputs_intermediate[k][0],outputs_dense[k][0]),2)
        outputs[k] = torch.relu(output)
    ...
    #outputs_2 = outputs_2[:,:,:1,:] #12 layers, [mask]
    ...
    dir = "task_activated_neuron_plm"
    if os.path.isdir(dir):
        save_dir = dir+"/"+save_name
        if os.path.isdir(save_dir):
            torch.save(outputs,save_dir+"/task_activated_neuron")
            #torch.save(outputs_2,save_dir+"/task_activated_neuron_2")
        else:
            os.mkdir(save_dir)
            torch.save(outputs,save_dir+"/task_activated_neuron")
            #torch.save(outputs_2,save_dir+"/task_activated_neuron_2")
